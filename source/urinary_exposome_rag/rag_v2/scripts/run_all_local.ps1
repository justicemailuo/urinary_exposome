...
Start-Process -FilePath powershell -ArgumentList @(
  ...
  "Bypass",
  "-Command",
  "cd '$Project'; `$env:RAG_API_BASE_URL='http://127.0.0.1:8890'; `$env:STREAMLIT_BROWSER_GATHER_USAGE_STATS='false'; python -m streamlit run .\streamlit_app\app.py --server.address 127.0.0.1 --server.port 8501 --server.headless true --browser.gatherUsageStats false *> '$StreamlitLog'"
) -WindowStyle Hidden

Start-Sleep -Seconds 6

Write-Host "Local Streamlit: http://127.0.0.1:8501"
